 ...
 * @Description:
 * @Author: liweihai
 * @Date: Created in 2019/1/9 22:16
 */
@Component
public class RedisUtil {
    ...
    public boolean setLogin(String key, Object value, int time) {
        try {
            if (time > 0) {
    ...
    public Object increment(String key, Long value) {
        return key == null ? null : redisTemplate.opsForValue().increment(key, value);
    }

    public synchronized boolean lock(String lockKey, String value, long time) {
        /*该方法会在没有key时，设置key;存在key时返回false；因此可以通过该方法及设置key的有效期，判断是否有其它线程持有锁*/

        Boolean success = redisTemplate.opsForValue().setIfAbsent(lockKey, value, time, TimeUnit.SECONDS);
        return  success;

    }
    //事务锁
    public synchronized boolean sessionLock(String lockKey, String value, long time) {
